...
<br>The MAE on the other hand does not square the difference between forecast and actual variance valeus but calculated the mean absolute difference between forecast and actual values. Hence it penalizes extreme day misses less than the MSE where the forecast errors are relatively large.

$$QLIKE = \frac{1}{T}\sum_{t=1}^{T}\left(\frac{r_t^2}{\hat{\sigma}^2_t} - \log\frac{r_t^2}{\hat{\sigma}^2_t} - 1\right)$$
<br>The QLIKE test is the preferred metric for variance forecast evaluations as it penalizes overestimation and underestimation of volatility differently just as how a market behaves. Secondly, it is guaranteed to correctly rank a model by specification even under noisy proxies such as squared returns.
